...
interface Props {
  ...
  onChange: (countries: Array<Country>) => void;
}

export function VirtualizedCountriesAutocomplete({ label, folderName, value, values, onChange }: Props) {
  const intl = useIntl();
  ...
  function renderRow(props: ListChildComponentProps) {
    ...
    return (
      <ListItem {...dataSet.props} style={inlineStyle} key={index}>
        <ListItemButton>
          {folderName && option && option.flagFile && (
            <ListItemAvatar>
              <Avatar src={getImageUrl(folderName, option.flagFile)} />
            </ListItemAvatar>
          )}
  ...
  return (
    <Autocomplete
      ...
      renderInput={(params) => <TextField {...params} variant="outlined" label={intl.formatMessage({ id: label })} />}
      renderTags={(options, getTagProps) => {
        return options.map((option, index) => {
          if (folderName && option.flagFile) {
            return (
              <Chip
                key={option.tag + "chip"}
                {...getTagProps}
                ...
                avatar={<Avatar src={getImageUrl(folderName, option.flagFile)} />}
              />
            );
          } else {
            return (
